...
{
	if ((int)a.getGrade() > this->getGradeToSign())
		throw (Bureaucrat::GradeTooLowException());
	else if (this->getSigned())
		std::cout << this->getName() << " Form has Already Been Signed." << std::endl;
	else
	{
		std::cout << "Form " << this->getName() << " signed by " << a.getName() << std::endl;
		this->_signed = 1;
	}
}
...
{
	std::string _signed = "No\n";

	if (a->getSigned())
		_signed = "Yes\n";
	o << a->getName() << ":\n\tSigned :\t\t\t" << _signed << "\tGrade Needed to Sign :\t\t" << a->getGradeToSign()
		<< "\n\tGrade Needed to Execute :\t" << a->getGradeToExecute() << std::endl;
	return (o);
}
